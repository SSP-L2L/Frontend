...
App.factory('VesselProcessService', ['$http', '$q', function ($http, $q) {  
  return {  
	  ...
	  GetProcessVariablesById : function(pid) {  
	      ...
	      var deferred = $q.defer(); // 声明延后执行，表示要去监控后面的执行  
	      $http.get(activityBasepath+'/runtime/process-instances/'+pid+"/variables").  
	      success(function(data, status, headers, config) {  
	   ...
	   * PutProcessVariables
	   */
	  PutProcessVariables : function(pid , vVariables) {  
		  console.log("************PutProcessVariables*********************");
	      var deferred = $q.defer(); // 声明延后执行，表示要去监控后面的执行  
	      $http.put(activityBasepath+'/runtime/vessel-procinst/'+pid+'/variables' , vVariables).  
	      success(function(data, status, headers, config) {  
