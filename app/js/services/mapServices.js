...
App.service('MapService', function (MapFactory, $http, Session, VesselProcessService, $interval) {
        ...
        this.doNavigation = function (event, ZoomInVal,wagonMarker) {
            $.toaster('车辆规划路径,目的地：' + event.data.vDestPort.pname, 'Wagon', 'success');
            ...
            let expandPath = function () {
                ...
                if (navg1.getNaviStatus().toString() === 'pause' && navg1.isCursorAtPathEnd()) {
                    if (index + 1 > searchPathData.length - 1) {
                        ...
                        // console.log("passDistanceData",passDistanceData);
                        // if (gpTimer !== null) {
                        //     $interval.cancel(gpTimer);
                        // }
                        if (wagonMarker !== null) {
            ...
            let getPosition = function () {
                ...
                for(let i=0;i<index;i++){
                    passDistanceData+=parseInt(searchDistanceData[i]);
                }
                passDistanceData+=Math.round(navg1.getMovedDistance());
                // console.log("passDistanceData",passDistanceData);
